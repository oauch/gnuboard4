...
<tr>
    ...
    <td class="admin_basic_board_writetd">관리자 E-mail</td>
    <td><input type=text name='mb_nick' maxlength=90 minlength=2 required itemname='관리자 E-mail' value='<? echo $mb[mb_nick] ?>'></td>
</tr>
<tr>
...
<p align=center>
    ...
    <input type=button value='  목  록  ' onclick="document.location.href='./member_list.php?<?=$qstr?>';">&nbsp;
    
    <? if ($w != '') { ?>
    <input type=button value='  삭  제  ' onclick="del('./member_delete.php?<?=$qstr?>&w=d&mb_id=<?=$mb[mb_id]?>&url=<?=$_SERVER[PHP_SELF]?>');">&nbsp;
    <? } ?>
</form>
