...
for ($i=0; $i<count($_FILES[bf_file][name]); $i++) 
{
    ...
    if (is_uploaded_file($tmp_file)) 
    {
        ...
        if (preg_match("/\.(jp[e]?g|gif|png)$/i", $_FILES[bf_file][name][$i]) && file_exists($dest_file) && $bo_table=="kyungnam_info_548623")
        {
...
if ($w == "" || $w == "r") 
{
    ...
    {
        $mb_id = $member[mb_id];
        if(!$wr_name) $wr_name = $board[bo_use_name] ? $member[mb_name] : $member[mb_nick];
