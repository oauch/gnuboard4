...
<tr height="25">
    ...
    <td align="right" style="font:normal 11px tahoma; color:#BABABA;">
        ...
        <? if ($write_href) { ?><span id=basicclas01><a href='<?=$admin_href?>' id="board_button2">Admin</a></span><? } ?></td>
</tr>
...
{ 
	...
	if (file_exists($thumb)){
		$img = "<img src='$thumb' border=0 width='180' height='230'>";
	}
	...
	echo "<tr valign='top'>";
	echo "<td style='border:solid 1px #dfdfdf; width:180; height:230;' align='center'>";
	echo "<table width='180' height='230' cellpadding='0' cellspacing='0' border='0'>
	      <td align='center'>{$view_href}<span style='display: block;'>{$img}</span></a>
              <table width='100%' cellpadding='0' cellspacing='0' style='padding:15px 0px 10px 0px;'>
	      <tr>
	      <td id='prf-name'><a href='{$list[$i][href]}'>{$list[$i][subject]}</a></td>
	      <td align='right' id='more-bt' valign='top'><a href='{$list[$i][href]}'>more</a></td>
	      </tr>
	      </table>
              </td>
	      </tr></table>";
	...
	if ($is_checkbox) {
		echo "<tr align=center><td>$checkbox <a href=\"javascript:rank('up','".$list[$i][wr_id]."')\"><img src=\"{$board_skin_path}/img/btn_up.gif\" alt='한칸위로' border='0'></a> <a href=\"javascript:rank('down','".$list[$i][wr_id]."')\"><img src=\"{$board_skin_path}/img/btn_down.gif\" alt='한칸아래로' border='0'></a></td></tr>";
	}
...
if ($i == 0)
    echo "<td colspan='$board[bo_gallery_cols]' height=50 align=center>There are no posts.</td>";
?>
</form>
</tr>
</table>
...
<div style="clear:both; margin-top:7px; height:31px;">
    ...
    <? if ($write_href) { ?><div id=basicclas01><a href='<?=$write_href?>' id="board_button2">Write</a></div><? } ?>
    </div>
</div>
