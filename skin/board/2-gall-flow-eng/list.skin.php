...
.grid {min-height:100px; border:1px solid #dedede; margin:8px; float:left;}
.grid strong {display:block; padding:3px; font-size:11pt; padding:0px 10px 10px 10px;}
.grid .imgholder img {width:100%; border:1px solid #f7f5f5; display:block;}
.grid .imgholder {padding:10px;}
</style>
...
<!-- 목록 배치 시작 -->
<div id='container' style="{$td_width}%">
<?
...
{ 
	...
	echo "			{$view_href}{$img}</a>";
	echo "		</div>";
	echo "		<strong>{$list[$i][subject]}</strong>";
	if ($is_checkbox) {
		echo "<div>$checkbox <a href=\"javascript:rank('up','".$list[$i][wr_id]."')\"><img src=\"{$board_skin_path}/img/btn_up.gif\" alt='한칸위로' border='0'></a> <a href=\"javascript:rank('down','".$list[$i][wr_id]."')\"><img src=\"{$board_skin_path}/img/btn_down.gif\" alt='한칸아래로' border='0'></a></div>";
	}	
	echo "	</div>";
    ...
    */
    ...
    <div class="board_search" style="padding-top:10px;">
        ...
        <select name="sfl">
            ...
            <option value="wr_subject||wr_content">Subject+Conten</option>
...
<script src="<?=$board_skin_path?>/blocksit.min.js"></script>
<script>
$(document).ready(function() {
	...
	$(window).load( function() {
		$('#container').BlocksIt({
			numOfCol: <?=$board[bo_gallery_cols]?>,   /* 가로목록갯수 */
			offsetX: 8,
			offsetY: 8
		});
	});			
