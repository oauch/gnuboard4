...
<div>
    ...
    <tr>
        <td class="viewtit">제목</td>
	<td>
            <? if ($is_category) { echo ($category_name ? "[$view[ca_name]] " : ""); } ?>
            <?=cut_hangul_last(get_text($view[wr_subject]))?>
        </td>
        <td class="viewtit">등록일</td>
        <td width="110">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <table width="100%" border="0" cellspacing="5" cellpadding="0" class="view_text">
      <tr>
        <td class="view_text2">제품명</td>
        <td>&nbsp;<?=cut_hangul_last(get_text($view[wr_subject]))?>&nbsp;</td>
      </tr>
      <tr>
        <td class="view_text2">모델명</td>
        <td>&nbsp;<?=$view[wr_2]?>&nbsp;</td>
      </tr>
      <tr>
        <td class="view_text2">사이즈</td>
        <td>&nbsp;<?=$view[wr_3]?>&nbsp;</td>
      </tr>
      <tr>
        <td class="view_text2">특징</td>
        <td>&nbsp;<?=$view[wr_4]?>&nbsp;</td>
      </tr>
        <?
        // 가변 파일
        ...
        for ($i=0; $i<count($view[file]); $i++) {
            if ($view[file][$i][source] && !$view[file][$i][view]) {
                $cnt++;
                echo "<tr><td class='view_text2'>첨부파일</td>";
        ...
        for ($i=1; $i<=$g4[link_count]; $i++) {
            if ($view[link][$i]) {
                ...
                echo "<tr><td class='view_text2'>링크</td>";
